...
import { checkPriceWarning, calculateReferenceValue } from '../utils/rulesEngine';
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#DDEBD8] mt-0.5">
                {collectorProfile.company} · {collectorProfile.location} · ID: {collectorProfile.id}
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-[#3F7655]/15">
          ...
          >
            ...
            <span>Reference Scrap Prices (Rule Dataset)</span>
          </button>
        </div>
          ...
          <div className="mt-6 space-y-5">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="relative w-full sm:w-80">
                ...
                <input
                  type="text"
                  placeholder="Search requirement category or city..."
                  value={searchFilter}
              ...
              <span className="text-xs font-bold text-[#718078]">
                Showing {filteredRequirements.length} active demand listings
              </span>
            </div>
                ...
                >
                  <div className="space-y-3">
                    ...
                    <div className="bg-[#FAF8F2] p-3 rounded-2xl border border-[#3F7655]/15 grid grid-cols-2 gap-2 text-center">
                      <div>
                        <span className="text-[10px] text-[#718078] font-bold block">Target Quantity</span>
                        <span className="text-sm font-black text-[#244936]">{req.requiredQuantityKg} kg</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-[#718078] font-bold block">Budget Rate</span>
                  ...
                  <div className="pt-3 border-t border-[#3F7655]/10 flex items-center justify-between gap-2">
                    <span className="text-[11px] font-bold text-slate-500">
                      Ref: ₹{req.referenceMin}–₹{req.referenceMax}/kg
                    </span>
                    <button
              ...
              {materialLots.map(lot => (
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-[#FAF8F2] p-3 rounded-2xl border border-[#3F7655]/15 text-center">
                    ...
                    <div>
                      <span className="text-[10px] text-[#718078] font-bold block">Lot Value</span>
                      <span className="text-sm font-black text-[#3F7655]">₹{lot.estimatedLotValue.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-[10px] text-[#718078] font-bold block">Recycler</span>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    <button
                      ...
                      className="px-4 py-2 text-xs font-bold text-[#244936] bg-[#DDEBD8] hover:bg-[#c9e0c1] rounded-xl transition flex items-center gap-1.5"
                    >
                      <QrCode className="w-4 h-4 text-[#3F7655]" />
                      <span>{t("viewDetails")} & QR</span>
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="mt-6 space-y-4">
            <div className="bg-white rounded-[28px] border border-[#3F7655]/20 p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-[#3F7655]/15">
                <div>
                  <h3 className="text-base font-black text-[#244936]">Scrap Reference Price Dataset</h3>
                  <p className="text-xs text-[#718078]">
                    Deterministic baseline rates for e-waste lot calculations. Source: Demo Scrap Price Dataset (Updated 10 Sep 2026).
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {referenceScrapPrices.map(item => (
                  <div key={item.id} className="bg-[#FAF8F2] p-4 rounded-2xl border border-[#3F7655]/15 space-y-2">
                    ...
                    <h4 className="font-extrabold text-sm text-[#203128]">{item.material}</h4>
                    <span className="text-[11px] font-bold text-slate-500 block">{item.category} · Ref Range: ₹{item.referenceMin}–₹{item.referenceMax}/kg</span>
