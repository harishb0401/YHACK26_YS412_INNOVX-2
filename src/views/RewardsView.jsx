...
import { rewardsList } from '../mockData';
...
export default function RewardsView({ setActiveView, currentPoints = 1240 }) {
  ...
  const handleRedeem = (reward) => {
    if (currentPoints >= reward.pointsRequired) {
      ...
      alert(`🎉 Congratulations! You have successfully redeemed "${reward.title}" for ${reward.pointsRequired} points. Confirmation code sent to your registered mobile!`);
    } else {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="bg-[#244936] text-white p-8 sm:p-10 rounded-[32px] shadow-xl border border-[#3F7655]/40 text-center space-y-6 relative overflow-hidden">
          ...
          <div className="space-y-1">
            <span className="text-xs font-bold uppercase tracking-wider text-[#F2C94C]">{t("rewardsBalanceTitle")}</span>
            <h1 className="text-4xl sm:text-5xl font-black text-white tracking-tight">
              {currentPoints.toLocaleString()} <span className="text-2xl text-[#F2C94C]">{t("pointsUnit")}</span>
            </h1>
          </div>
          ...
          <div className="max-w-md mx-auto space-y-2 text-left">
            <div className="flex justify-between text-xs font-bold text-[#DDEBD8]">
              <span>{t("currentPointsLabel", { pts: currentPoints })}</span>
              <span>{t("targetPointsLabel", { pts: "1,500" })}</span>
            </div>
                ...
                >
                  ...
                  <div className="pt-4 border-t border-[#3F7655]/10 space-y-3">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-[#718078]">{t("targetQty")}:</span>
                    ...
                    {isAlreadyRedeemed ? (
                      <div className="w-full py-2.5 bg-emerald-100 text-emerald-800 font-bold text-xs rounded-xl text-center flex items-center justify-center gap-1">
                        <CheckCircle2 className="w-4 h-4" /> {t("redeemedSuccess")}
                      </div>
                    ) : canAfford ? (
                      <button
                      ...
                      >
                        <Sparkles className="w-3.5 h-3.5" /> {t("redeemNowBtn")}
                      </button>
                    ) : (
                      <button
                        onClick={() => handleRedeem(reward)}
                      ...
                      >
                        <Lock className="w-3.5 h-3.5" /> {t("needMorePointsBtn", { pts: reward.pointsRequired - currentPoints })}
                      </button>
                    )}
                  </div>
                </div>
