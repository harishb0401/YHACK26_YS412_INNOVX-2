    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-[#203128] mt-1.5">My E-Waste Requests</h1>
          <p className="text-xs sm:text-sm text-[#718078]">
            Track your declared e-waste requests, review incoming offers from authorized recyclers, and manage handovers.
          </p>
        </div>
      ...
      <div className="bg-white p-4 sm:p-5 rounded-[28px] border border-[#3F7655]/20 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            placeholder="Search request ID, material, or location..."
      ...
      {filteredRequests.length > 0 ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-[#718078]">
                      <div className="p-2.5 bg-[#F8F5EA] rounded-xl">
                        <span className="text-[10px] text-[#718078] block uppercase font-bold">Quantity</span>
                        <span className="text-sm font-black text-[#203128]">{qty} {unit}</span>
                      </div>

                      <div className="p-2.5 bg-[#F8F5EA] rounded-xl">
                        <span className="text-[10px] text-[#718078] block uppercase font-bold">Submitted Date</span>
                    ...
                    <div className="p-3 bg-[#FAF8F2] rounded-2xl border border-[#3F7655]/15 flex items-center justify-between text-xs">
                      <div>
                        <span className="text-[10px] font-extrabold uppercase text-[#718078] block">Received Offers</span>
                      ...
                      <div className="text-right">
                        <span className="text-[10px] font-extrabold uppercase text-[#718078] block">Benchmark</span>
                    ...
                    {request.selectedRecyclerName && (
                      <div className="p-2.5 bg-teal-50 border border-teal-200 rounded-xl text-xs text-teal-900 font-bold flex items-center gap-2">
                        ...
                        <span className="truncate">Accepted: {request.selectedRecyclerName}</span>
                      </div>
                    )}
                  </div>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-[#FAF8F2] border-b border-[#3F7655]/15 text-[#203128] font-black uppercase text-[11px] tracking-wider">
                  <tr>
                    ...
                    <th className="px-6 py-4">Offers Received</th>
                    ...
                    return (
                      <tr key={request.id} className="hover:bg-[#F8F5EA]/70 transition">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-[11px] text-[#718078]">{request.category}</div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          {reqOffers.length > 0 ? (
                            <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 rounded-full font-bold text-[11px] inline-flex items-center gap-1">
                              ...
                              {reqOffers.length} {reqOffers.length === 1 ? 'Offer' : 'Offers'}
                            </span>
                          ) : (
                            <span className="text-[#718078] text-[11px] italic">0 (Awaiting)</span>
                          )}
                        </td>
        ...
        <div className="bg-white p-12 rounded-[32px] border border-[#3F7655]/20 text-center space-y-4 shadow-sm">
          ...
          <h3 className="text-lg font-black text-[#203128]">No E-Waste Requests Found</h3>
          <p className="text-xs text-[#718078] max-w-sm mx-auto">
            {searchQuery 
              ? "No requests matched your filter parameters. Try clearing your search query." 
              : "You have not submitted any e-waste requests yet. Create a new request to notify authorized recyclers in your area."}
          </p>
          <button
            onClick={() => navigate('/collector/register-waste')}
            className="px-6 py-2.5 bg-[#3F7655] hover:bg-[#244936] text-white rounded-xl font-bold text-xs shadow transition cursor-pointer"
          >
            Create First E-Waste Request
          </button>
        </div>
      )}
    </div>
