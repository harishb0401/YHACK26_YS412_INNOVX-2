...
export default function RecyclerProfile({ recyclerProfile = mockRecycler }) {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {/* Header */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-[#203128]">Recycler Facility Profile</h1>
        <p className="text-xs sm:text-sm text-[#718078]">
          Manage your processing facility credentials, authorized waste categories, and CPCB license details.
        </p>
      </div>
      ...
      {savedSuccess && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-2xl text-xs font-bold flex items-center gap-2">
          ...
          <span>Facility details updated successfully!</span>
        </div>
      )}
      ...
      <div className="bg-white p-6 sm:p-10 rounded-[32px] border border-[#3F7655]/20 shadow-md space-y-8">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-extrabold text-[#203128] block mb-1">Company / Facility Name</label>
              <input
                type="text"
                disabled={!isEditing}
            ...
            <div>
              <label className="text-xs font-extrabold text-[#203128] block mb-1">CPCB / TNPCB Registration No</label>
              <input
                type="text"
                disabled={!isEditing}
          ...
          <div>
            <label className="text-xs font-extrabold text-[#203128] block mb-1">Facility Address</label>
            <input
              type="text"
              disabled={!isEditing}
